    ...
    return (
      <View style={{ display: 'flex', flex: 1, backgroundColor: Colors.primaryDark }}>
        ...
        <ScrollView>
          {
            exercises.map((x, i) => {
              ...
              return (
                <TouchableOpacity onLongPress={() => this.onLongPress(x)} onPress={() => this.onAddExercise(x)} key={i} style={{ margin: 2, marginLeft: 10 }}>
        ...
        <TouchableOpacity onPress={() => this.onAddExercises()}>
          <H3 style={{height: 50, backgroundColor: Colors.primaryLight, display: 'flex', alignSelf: 'center'}}>Dodaj Ćwiczenia</H3>
        </TouchableOpacity>
      </View>
    )
  }
